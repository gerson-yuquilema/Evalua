...
const EvaluationFlow = () => {
  ...
  const simulateEvaluation = async () => {
    try {
      ...
      if (!formData.ruc) {
        toast({ title: "Falta RUC", description: "Ingresa el RUC antes de evaluar.", variant: "destructive" });
        return;
      }
      if (!formData.files?.length) {
        toast({ title: "Sin archivos", description: "Debes subir los 3 archivos requeridos.", variant: "destructive" });
        return;
      }

      const lower = (f: File) => f.name.toLowerCase();
      const balances = formData.files.find((f) => lower(f).includes("balance") || lower(f).endsWith(".txt"));

      const csvs = formData.files.filter((f) => lower(f).endsWith(".csv"));
      const referencias = csvs.find((f) => lower(f).includes("refer")) ?? csvs[0];
      const digitales = csvs.find((f) => lower(f).includes("digital")) ?? csvs.find((f) => f !== referencias);

      if (!balances || !referencias || !digitales) {
        toast({
          title: "Faltan archivos",
          description: "Debes subir: balances (.txt), referencias (.csv) y datos digitales (.csv).",
          variant: "destructive",
        });
        return;
      }
      ...
      form.append("ruc_objetivo", formData.ruc);
      form.append("balances_file", balances);
      form.append("referencias_file", referencias);
      form.append("datos_digitales_file", digitales);
      ...
      toast({ title: "Evaluación Completada", description: `Score generado: ${score}/100` });
    } catch (e: any) {
      console.error("simulateEvaluation fail:", e);
      toast({ title: "Error al evaluar", description: e?.message || "No se pudo obtener el score.", variant: "destructive" });
    } finally {
  ...
  const handleDescargarReporte = () => {
    ...
    const colorRiesgoRGB: [number, number, number] =
      score >= 80 ? [67, 160, 71] : score >= 60 ? [251, 192, 45] : [255, 152, 0];
    const [cr, cg, cb] = colorRiesgoRGB;
    ...
    doc.text(String(score), 180, 50, { align: "center" as any });
    doc.setFontSize(10);
    doc.text("SCORE", 180, 58, { align: "center" as any });
    doc.setFontSize(12);
    doc.setTextColor(0, 0, 0);
    ...
    autoTable(doc, {
      ...
      body: [
        [
          { content: "Ventas Mensuales", styles: { fillColor: [245, 245, 245] } },
          { content: "$45,230", styles: { textColor: [26, 35, 126], fontStyle: "bold" } },
          { content: "Liquidez", styles: { fillColor: [245, 245, 245] } },
          { content: "68%", styles: { textColor: [26, 35, 126], fontStyle: "bold" } },
        ],
        [
          { content: "Clientes Activos", styles: { fillColor: [245, 245, 245] } },
          { content: "156", styles: { textColor: [26, 35, 126], fontStyle: "bold" } },
          { content: "Reputación Digital", styles: { fillColor: [245, 245, 245] } },
          { content: "79/100", styles: { textColor: [26, 35, 126], fontStyle: "bold" } },
        ],
      ],
    ...
    let y = (doc as any).lastAutoTable?.finalY ? (doc as any).lastAutoTable.finalY + 10 : 100;
    doc.setFontSize(12);
    ...
    doc.text(["⚠️ Requiere Revisión:", "Se requiere análisis adicional para determinar elegibilidad."], 12, y + 8);
    doc.setTextColor(0, 0, 0);
    ...
    if (formData.files.length > 0) {
      formData.files.forEach((file, idx) => doc.text(`• ${file.name}`, 15, y + 8 + idx * 6));
    } else {
            ...
            <div className="grid md:grid-cols-2 gap-4 text-sm text-muted-foreground">
              <div className="space-y-2">
                ...
                <ul className="space-y-1">
                  <li>• balances (.txt)</li>
                  <li>• referencias (.csv)</li>
                  <li>• datos digitales (.csv)</li>
                </ul>
              </div>
              <div className="space-y-2">
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  className="mt-1"
                />
                <p className="text-sm text-muted-foreground mt-1">Facebook, Instagram, LinkedIn o sitio web oficial</p>
              </div>
              <div>
              ...
              <div>
                ...
                <Input
                  ...
                  className="mt-1"
                />
                <p className="text-sm text-muted-foreground mt-1">Ingresa el enlace al perfil de X (Twitter) de la empresa.</p>
              </div>
              <div>
                ...
                <Input
                  ...
                  className="mt-1"
                />
                <p className="text-sm text-muted-foreground mt-1">Ingresa el ID del tweet para analizar los comentarios.</p>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-muted-foreground">
                {isEvaluating ? "Analizando datos..." : evaluationResult != null ? "Evaluación completada" : "Inicie el análisis inteligente"}
              </p>
            </div>
            ...
            {!isEvaluating && evaluationResult == null && (
              <div className="text-center space-y-6">
                ...
                <Button onClick={simulateEvaluation} disabled={isEvaluating} className="bg-gradient-primary hover:opacity-90 text-lg px-8 py-6" size="lg">
            ...
            {evaluationResult !== null && (
              <div className="text-center space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <Button className="bg-gradient-primary hover:opacity-90" onClick={handleDescargarReporte}>
                    Descargar Reporte
                  </Button>
                </div>
              </div>
      ...
      <div className="max-w-5xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="col-span-1 bg-white rounded-lg p-4 flex flex-col justify-between shadow-sm">
            ...
            <div className="text-green-500 text-xs mt-1">
              {variacionVentas > 0 ? `▲ +${variacionVentas}%` : `▼ ${variacionVentas}%`}
            </div>
          </div>
          ...
          <div className="col-span-1 bg-white rounded-lg p-4 flex flex-col justify-between shadow-sm">
            ...
            <div className="text-green-500 text-xs mt-1">
              {variacionClientes > 0 ? `▲ +${variacionClientes}%` : `▼ ${variacionClientes}%`}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg p-6 mb-6 shadow-sm">
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <div>
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div className="h-2 rounded-full bg-blue-500" style={{ width: `${liquidez}%` }} />
              </div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground mb-1">Rentabilidad</div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div className="h-2 rounded-full bg-blue-500" style={{ width: `${rentabilidad}%` }} />
              </div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground mb-1">Reputación Digital</div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div className="h-2 rounded-full bg-blue-500" style={{ width: `${reputacionDigital}%` }} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="bg-white rounded-lg p-6 shadow-sm">
            ...
            <div className="text-2xl font-bold text-blue-700 mb-2">$25,000</div>
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm">
            ...
            <ul className="text-sm space-y-2">
              <li>Si aumenta ventas 20%: <span className="font-bold text-green-600">{score + 6} (+6)</span></li>
              <li>Si mejora reputación digital: <span className="font-bold text-green-600">{score + 3} (+3)</span></li>
              <li>Si reduce deudas 30%: <span className="font-bold text-green-600">{score + 8} (+8)</span></li>
            </ul>
          </div>
        </div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
            ...
            <div key={step.number} className="flex items-center">
              <div
                className={`flex items-center justify-center w-12 h-12 rounded-full border-2 transition-colors ${
                  currentStep >= step.number ? "bg-primary border-primary text-primary-foreground" : "border-border text-muted-foreground"
                }`}
              >
              ...
              {index < steps.length - 1 && <Separator className="w-12 mx-4 hidden md:block" />}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex justify-between mt-6">
        ...
        <Button onClick={handleNext} disabled={currentStep === 4 || (currentStep === 4 && isEvaluating)} className="bg-gradient-primary hover:opacity-90">
